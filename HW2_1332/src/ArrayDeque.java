...
public class ArrayDeque<T> {
    ...
    public void addFirst(T data) {
        if (data == null) {
            throw new IllegalArgumentException("ERROR: Data is null. Please include some Object or Data.");
        }

        if (size == backingArray.length) {
            T[] temp = (T[]) new Object[2 * backingArray.length];
            for (int i = mod(2 * front, backingArray.length) + 1; i != mod(front - 1,
                    getBackingArray().length); i = mod(i + 1, getBackingArray().length)) {
                temp[i + 1] = backingArray[i];
            }
            temp[0] = data;
            temp[size] = backingArray[mod(front - 1, getBackingArray().length)];
            backingArray = temp;
            front = 0;
        } else {
    ...
    public T removeFirst() {
        if (size == 0) {
            throw new NoSuchElementException("ERROR: The array is empty, add data or objects before trying to remove"
                    + "elements.");
        }

        T dataReturn = backingArray[front];
    ...
    public T removeLast() {
        ...
        T dataReturn = backingArray[mod(front + size - 1, backingArray.length)];
